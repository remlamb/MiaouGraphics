...
namespace gpr5300 {
class HelloIBL final : public Scene {
 public:
  void Begin() override;
  void End() override;
 ...
 private:
  const std::string_view textVertexShaderFilePath_ =
      "data/shaders/hello_IBL/pbr.vert";
  const std::string_view textFragmentShaderFilePath_ =
      "data/shaders/hello_IBL/pbr.frag";
...
void HelloIBL::Begin() {
  ...
  pbr_pipeline.SetVec3("albedo", 0.5f, 0.0f, 0.0f);
  pbr_pipeline.SetFloat("ao", 1.0f);
